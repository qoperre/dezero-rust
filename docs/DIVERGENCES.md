...
| 32 | VGG16 pretrained weights (step 58) | `pretrained=True` fetches a 528 MB ImageNet `.npz` over HTTP | architecture only; `Vgg16::new` starts from fresh weights | No HTTP client (row 25) and the file is numpy's format, which this port does not read (row 31). Weights saved by this port load normally. Python's `preprocess` is image handling, not a network, and is also absent. |
